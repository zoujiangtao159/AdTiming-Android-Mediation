...
public class AppLovinAdapter extends CustomAdsAdapter implements AppLovinAdVideoPlaybackListener
    ...
    public String getAdapterVersion() {
        return MediationInfo.ADAPTER_7_VERSION;
    }

    @Override
    public int getAdNetworkId() {
        return MediationInfo.PLAT_ID_7;
    }
        ...
        if (TextUtils.isEmpty(error)) {
            if (mAppLovinSDk != null) {
                mAppLovinSDk.getAdService().loadNextAdForZoneId(adUnitId, new AppLovinAdLoadListener() {
                    ...
                    @Override
                    public void failedToReceiveAd(int i) {
                        AdLog.getSingleton().LogE("Adt-AppLovin: AppLovin interstitial ad load failed : " + i);
                        if (callback != null) {
                            callback.onInterstitialAdLoadFailed("Adt-");
                        }
                    }
                });
            }
    ...
    public boolean isInterstitialAdAvailable(String adUnitId) {
        AppLovinInterstitialAdDialog adDialog = mIsAds.get(adUnitId);
        return adDialog != null && adDialog.isAdReadyToDisplay() && mAppLovinIsAds.containsKey(adUnitId);
    }
